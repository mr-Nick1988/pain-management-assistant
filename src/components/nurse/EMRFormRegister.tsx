    ...
    return (
        <div className="p-6">
            <FormCard
                ...
                isLoading={isLoading}
                error={error &&
                    <ErrorMessage message={("data" in error && error.data && typeof error.data === "object" && "message" in error.data ? String(error.data.message) : undefined) || "Failed to save EMR"}/>}
            >
                <FormGrid columns={2}>
                    <FormFieldWrapper label="Height (cm)" required hint="Enter 50–250 cm" error={errors.height}>
                        <Input type="number" name="height" placeholder="Height" value={form.height || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="Weight (kg)" required hint="Enter 20–300 kg" error={errors.weight}>
                        <Input type="number" name="weight" placeholder="Weight" value={form.weight || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="GFR" required hint="Enter A–E or 0–150 ml/min" error={errors.gfr}>
                        <Input type="text" name="gfr" placeholder="Kidney function" value={form.gfr}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="Child-Pugh Score" required hint="Enter A, B or C"
                                      error={errors.childPughScore}>
                        <Input type="text" name="childPughScore" placeholder="A/B/C" value={form.childPughScore}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="PLT" required hint="Enter 50–600 K/µL" error={errors.plt}>
                        <Input type="number" name="plt" placeholder="Platelet count" value={form.plt || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="WBC" required hint="Enter 2–30 ×10³/µL" error={errors.wbc}>
                        <Input type="number" name="wbc" placeholder="White blood cells" value={form.wbc || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="SAT (%)" required hint="Enter 85–100%" error={errors.sat}>
                        <Input type="number" name="sat" placeholder="Oxygen saturation" value={form.sat || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                    <FormFieldWrapper label="Sodium (mmol/L)" required hint="Enter 110–160 mmol/L"
                                      error={errors.sodium}>
                        <Input type="number" name="sodium" placeholder="Sodium" value={form.sodium || ""}
                               onChange={handleChange} required/>
                    </FormFieldWrapper>
                </FormGrid>
                <FormFieldWrapper label="Sensitivities" hint="Example: Paracetamol, Tramadol, Ibuprofen">
                    <Input type="text" name="sensitivities" placeholder="Comma-separated (e.g. Paracetamol, Tramadol)"
